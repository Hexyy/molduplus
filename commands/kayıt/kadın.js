...
member.roles.add(kadın.id, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`).catch(err => message.channel.send(`\`\`\`Bir hata oluştu: ${err}\`\`\``));
if(isim && n) member.setNickname(n+" "+isim, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`); 
if(isim && !n) member.setNickname(isim, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`);
if(!isim && n) member.setNickname(nn+member.user.username, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`);
...
`));
  ...
  member.roles.add(kadın.id, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`).catch(err => message.channel.send(`\`\`\`Bir hata oluştu: ${err}\`\`\``));
member.roles.remove(kayıtsız.id, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`);
if(isim && n) member.setNickname(n+" "+isim, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`); 
if(isim && !n) member.setNickname(isim, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`);
if(!isim && n) member.setNickname(nn+member.user.username, `${message.author.tag} - Kadın Kayıt İşlemi Yapıldı.`);
