...
if(!role) return message.channel.send(new Discord.MessageEmbed().setTitle("<:hata:813391295665930260> Hata!").setColor("RED").setDescription(`**${message.author.username}** Bir rol etiketlemeyi unuttunuz.`)).then(a => a.delete({timeout: 10000}));
kayit.set(`kadın.${message.guild.id}`, role.id);
message.channel.send(nn.setTitle(`Görev Tamamlandı!`).setColor("#22BF41").setDescription(`${tik} Kayıt sistemi kadın rolünü ${role} rol olarak ayarladım.`).setColor(AloneDogru)).then(a => a.delete({timeout: 10000}));
